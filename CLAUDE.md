...
- Before changing anything in `templates/` or `KERNEL.md`, check the change against both `books/the-watch/` and `books/cadriharetha/`. If it only holds for one, it's a book rule and belongs in that book's `bible/`.
